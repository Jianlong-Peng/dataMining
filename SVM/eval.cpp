...
//self added at 2011-10-11
void validation(const dvec_t& dec_values, const ivec_t& ty, double *result);
...
const char *methods[] = {"precision","recall","fscore","BACC","AUC","ACC"};  //added by jlpeng
...
double fscore(const dvec_t& dec_values, const ivec_t& ty){
	...
	if(precision + recall == 0){
		//printf("warning: precision + recall = 0.\n");
		fscore = 0;
	}else
...
{
	/*
	**result: vector of length at least 3
	**result[0]: sensitivity; result[1]: specificity; result[2]: MCC
	*/
	size_t i;
	...
	result[0] = (double)tp/(tp+fn);
	result[1] = (double)tn/(tn+fp);
	result[2] = (double)(tp+tn)/(tp+fn+tn+fp);
	result[3] = (double)(tp*tn-fp*fn)/sqrt((tp+fn)*(tp+fp)*(tn+fp)*(tn+fn));
}

double *binary_class_cross_validation(const svm_problem *prob, const svm_parameter *param, 
	int nr_fold, int valid_method, int verbose)
{
/*
**self add int valid_method on 2011-09-26
**`int verbose` added at 2012-06-14
**valid_method:: 1~6. For other values, it will use AUC as validation method
*/
	int i;
	...
	dvec_t dec_values;
	ivec_t ty;
	double *result = Malloc(double, 5);
	//result[0]: sensitivity; result[1]: specificity; result[2]: accuracy; result[3]: MCC result[4]:?
...
//	srand(0);
	...
	if(verbose) {
		std::cout << "NO. true_label pred_label" << std::endl;
		for(i=0; i<l; i++)
			std::cout << perm[i]+1 << "  " << ty[i] << " " << ((dec_values[i])>=0?1:-1) << std::endl;
	}
	...
	switch(valid_method)
	{
		case 1: validation_function = precision; break;
		case 2: validation_function = recall; break;
		case 3: validation_function = fscore; break;
		case 4: validation_function = bac; break;
		case 5: validation_function = auc; break;
		case 6: validation_function = accuracy; break;
		default: validation_function = auc; break;
	}
	validation(dec_values, ty, result);
	result[4] = validation_function(dec_values, ty);
	
	return result;
}


double *binary_class_predict(FILE *input, FILE *output, struct svm_model* model, int valid_method, int predict_probability)
{
/*
**self add int valid_method at 2011-09-26
**possible values: 1~6. For other values except 1~6, it wil use AUC as validation method
*/
	int    total = 0;
	int    *labels;
	int    max_nr_attr = 64;
	...
	validation(dec_values, true_labels, result);
	result[4] = validation_function(dec_values, true_labels);

	return result;
}

//self added at 2011-09-26
//return the predicted y value given model and x
